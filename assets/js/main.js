...
function initEducationCarousel() {
    ...
    try {
        ...
        // Start auto-advance if user prefers no reduced motion
        ...
        updateCarousel = function() {
            ...
            const institution = currentCard.querySelector('.university')?.textContent || 
                               currentCard.querySelector('h4')?.textContent || '';
            const degree = currentCard.querySelector('h3')?.textContent || '';
            announcer.textContent = `Showing ${degree} from ${institution}, slide ${currentSlide + 1} of ${totalSlides}`;
        };
        
        console.log(`Education carousel initialized with ${totalSlides} slides`);
    } catch (error) {
        console.error('Error initializing education carousel:', error);
    }
}
